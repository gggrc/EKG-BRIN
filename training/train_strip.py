"""
train_strip.py — Latih U-Net segmentasi PER-LEAD target-BERSIH (metode Rahimi).

Input  : strip 1 lead (3x160x256). Target: HANYA trace lead itu (mask bersih).
Dengan OverlaySignal augmentation, model belajar mengabaikan trace tetangga yang
menumpuk -> menyelesaikan kasus V3 memanjat ke V2 di level MODEL (bukan tambalan).

Loss   : BCE + Dice. Output: checkpoints/strip_best.pt (out_ch=1).
Jalankan: python train_strip.py --epochs 40 --batch 32
"""
import os
import time
import argparse
...
def run(args):
    ...
    # sampler grouped per-record -> cache gambar 1x/record -> jauh lebih cepat
    ...
    net = UNet(in_ch=3, out_ch=1, base=args.base).to(dev)
    ...
    for ep in range(1, args.epochs + 1):
        ...
        if vd > best + 1e-4:
            ...
            torch.save({"model": net.state_dict(), "base": args.base,
                        "in_ch": 3, "out_ch": 1, "epoch": ep, "val_dice": vd},
                       os.path.join(CKPT_DIR, "strip_best.pt"))
        else:
            since += 1
            if since >= args.patience:
                print(f"early-stop ep {ep} (best Dice {best:.4f})"); break
    print(f"selesai. best val Dice {best:.4f} -> {CKPT_DIR}/strip_best.pt")
