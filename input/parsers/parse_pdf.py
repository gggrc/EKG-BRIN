...
def parse_pdf(file_path: str, vendor: str = "PDF Report") -> UniversalECG:
    ...
    # ── JALUR UTAMA: PDF VEKTOR (trace = polyline) -> sinyal EKSAK ──────
    # 12-lead (1 halaman) atau 6-lead Kardia (multi-halaman, disambung).
    # Tidak ada digitalisasi -> bebas artefak overlap/collision/box.
    try:
        from parsers.vector_pdf import extract_vector_pdf
    except Exception:
        from vector_pdf import extract_vector_pdf
    vec = extract_vector_pdf(file_path)
    if vec is not None:
        doc.close()
        fs = vec['fs']
        u = UniversalECG(
            leads=vec['leads'], sampling_rate=fs,
            duration_sec=vec['duration_sec'], num_leads=vec['num_leads'],
            input_format="pdf", device_vendor=vec['vendor'],
            calibration={
                'px_per_mV': vec['meta']['pt_per_mv'],
                'px_per_sec': vec['meta']['pt_per_sec'],
                'method': 'pdf_vector_exact', 'confidence': 1.0,
                'gain_mm_per_mV': 10.0, 'speed_mm_per_sec': 25.0,
                'highpass_hz': vec['meta'].get('highpass_hz'),
            },
        )
        u.metadata.patient_id = meta.get('patient_id', 'unknown')
        u.metadata.age = meta.get('age')
        u.metadata.sex = meta.get('sex')
        u.metadata.report = meta.get('rhythm')
        notes = [f"VEKTOR-EKSAK ({vec['meta']['source']})"]
        if 'heart_rate' in meta: notes.append(f"HR={meta['heart_rate']}bpm")
        if 'patient_name' in meta: notes.append(f"Name={meta['patient_name']}")
        u.notes = " | ".join(notes)
        print(f"  [OK] VEKTOR eksak: {vec['num_leads']} lead, "
              f"{vec['duration_sec']}s @ {fs}Hz ({vec['vendor']})")
        return u

    # ── FALLBACK: PDF raster -> render + digitalisasi (best-effort) ─────
